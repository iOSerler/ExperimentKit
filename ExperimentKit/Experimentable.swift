...
protocol Experimentable {
    ...
    @discardableResult
    func addExperiment<T: Decodable>(forKey key: String, as type: T.Type, action: @escaping (objectType, T) -> ()) -> objectType
}
...
extension NSObject: Experimentable {
    ...
    @discardableResult
    public func addExperiment<T: Decodable>(forKey key: String, as type: T.Type, action: @escaping (NSObject, T) -> ()) -> NSObject {
